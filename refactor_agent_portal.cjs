...
// 2. Remove the old activeWorkdeskTab declaration
const activeWorkdeskDeclaration = "const [activeWorkdeskTab, setActiveWorkdeskTab] = useState('MATERIAL_ORDER'); // Material views, Meter Installation, Discom Inspection";
content = content.replace(activeWorkdeskDeclaration, `// State moved to top`);

// 3. Simplify getWorkdeskCustomers
content = content.replace(
...
// 5. Replace handleSelectCustomerForStage
const handleSelectStart = '    const handleSelectCustomerForStage = (cust, stageTab) => {';
const handleSelectEnd = '        }\n    };';

const handleSelectStartIndex = content.indexOf(handleSelectStart);
const handleSelectEndIndex = content.indexOf(handleSelectEnd, handleSelectStartIndex);

if (handleSelectStartIndex !== -1 && handleSelectEndIndex !== -1) {
    content = content.substring(0, handleSelectStartIndex) + 
    `    const handleSelectCustomerForStage = (cust, stageTab) => {
        setEditData({});
        setActiveCustomerStage(stageTab);
        setSelectedCust(cust);
    };` + content.substring(handleSelectEndIndex + handleSelectEnd.length);
} else {
    console.error("Could not find handleSelectCustomerForStage");
    process.exit(1);
}

// 6. Replace the entire render block up to AddLeadModal
...
// 7. Make sure Modal is constrained with pl-[280px] to push it past the sidebar
content = content.replace(
...
console.log('Refactor complete v4');
