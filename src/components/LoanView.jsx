...
export default function LoanView({ customers, onSelectCustomer }) {
    ...
    const tagged = customers.filter(c => c.loan_tag);

    const grouped = LOAN_TAGS.reduce((acc, tag) => {
        const group = tagged.filter(c => c.loan_tag === tag.id);
    ...
    if (tagged.length === 0) return (
        <div className="flex flex-col items-center justify-center h-64 text-stone-400">
            ...
            <p className="font-semibold text-stone-500 text-sm">No loan tags active</p>
            <p className="text-xs text-stone-400 mt-1">Assign loan status tags inside customer detail cards.</p>
        </div>
    );
